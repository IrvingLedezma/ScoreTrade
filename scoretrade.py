...
probabilidades = {
    "Ticket": best_tickets,
    "Probabilidad (%)": [5.3, 10.2, 20.5, 88.4, 76.5, 93.1, 80.2, 82.7],
}
...
with st.sidebar:
    ...
    st.markdown(
        ...
        <div style='display: flex; align-items: center;'>
            <img src='{image_url}' style='width: 60px; height: 60px; margin-right: 30px;'>
...
def highlight_google(row):
    # Aplica el color amarillo si el valor en la columna "Ticket" es "GOOGL"
    return ['background-color: #5d6d7e' if ticket == 'GOOGL' else '' for ticket in row]

# Mostrar la tabla con formato
if selected_ticket != "":
    st.write(f"Probabilidad de éxito para el ticket {selected_ticket}:")

    selected_df = df_probabilidades

    styled_df = (
        selected_df.style
        .apply(highlight_google, subset=['Ticket'] ,   axis=1)  # subset=['Ticket'], Aplicar el color a la fila de GOOGL
        .format(precision=2)
        .bar(align="mid", color="#283747", vmin=0, vmax=100, subset=["Probabilidad (%)"])
        .set_properties(**{
...
"""
https://dev.to/barrisam/how-to-style-streamlit-metrics-in-custom-css-4h14
"""
...
with col1:
    st.metric(label="Score compra más", value="19%", delta="10%")
...
sub_df = load_dataset('WALMEX.MX')
#indicator_colors = {"SMA": "orange", "EMA": "violet", "WMA": "blue", "RSI": "yellow", "MOM": "black", "DEMA": "red", "MA": "tomato","TEMA": "dodgerblue"}
indicator_colors = {"SMA": "orange"}

st.bokeh_chart(create_chart(sub_df, True, True, []), use_container_width=True)
